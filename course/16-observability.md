...
The chapter above is the design. This section is what still breaks once that design is running — and the observability layer is special, because when *it* fails the failure is usually silent: nothing errors, the graphs just lie. They are ordered by how often they bite, not by how interesting they are. The first two go wrong on nearly every agent that ships telemetry; the last three start to matter once you have real traffic, real users, and real bills.

### Your metrics bill explodes and the dashboards get slow

*The symptom in one line: telemetry costs more than you expected, queries time out, and the metrics backend starts dropping data.*

This is the most common observability failure, and it grows from one innocent-looking decision: putting a per-run identifier on a metric. A metric *label* (also called a dimension — a key/value tag attached to a measurement, like `model="opus"`) creates one separate time series for every distinct value it takes. Put `run_id` or `session_id` on a metric and you mint a brand-new, never-repeated time series on every single run. This is *cardinality explosion* — cardinality being the count of distinct label combinations — and it is how teams discover their metrics backend bills by active series. The graphs slow to a crawl, the backend starts rejecting writes, and the irony is that the IDs which make *traces* powerful are exactly what poison *metrics*.

The fix is the **bounded-label rule**: a metric label may only carry values from a small, closed set — `model`, `tenant_id` if you have tens of tenants not millions, `tool_name`, `status`, `finish_reason`. Anything unbounded — `run_id`, `session_id`, `step_id`, free-text error messages, user input — goes on *traces and logs*, never on a metric label. When you need to jump from an aggregate metric to the individual run behind a spike, that is what **exemplars** are for: a metric data point carries a sample trace ID as an attachment, so you click the spike and land on a representative trace without ever making the trace ID a label. Put a hard ceiling on series count and alarm on it: track `active_series_count` per metric name and page when any one metric crosses a few thousand series — that alarm fires the day someone adds `run_id` to a label "just to debug something," which is exactly when you want to catch it.

### Your trace tree is a flat pile of disconnected spans

*The symptom in one line: instead of one run that branches into steps, model calls, and tools, you get hundreds of orphan spans that don't connect to anything.*

The chapter's trace tree assumes the parent-child links just happen. They don't — they happen only if *trace context* (the current span's ID, carried in ambient state so a child span knows who its parent is) propagates across every boundary the run crosses. Agents cross a lot of them: an `await` that hops threads, a tool call dispatched through a queue (Ch.15), a subagent spawned in a separate worker (Ch.10), a callback fired from a streaming SDK. At any boundary where context is not explicitly carried across, the child span starts a *new* root instead of attaching to its parent, and your beautiful tree fragments into a flat list. The most painful version is subagents: `parent_run_id` is set as an attribute (so the data is technically there) but the *span linkage* is broken, so the trace-replay UI shows the parent run with a mysteriously missing subtree and the subagent's work floating off on its own.

The fix has two halves. First, **propagate context explicitly across every async and process boundary** — when you cross a queue or spawn a subagent, serialize the trace context into the message or the child's startup args and re-attach it on the other side (OpenTelemetry calls this inject/extract; it is the same `traceparent` header that ties distributed services together). Second, **alarm on tree shape, not just span volume**: track the *orphan-span rate* — spans whose parent ID points at nothing in the same trace — and the *root-spans-per-run ratio*, which should be exactly 1.0. When a deploy breaks propagation, span volume stays flat (you're still emitting the same number of spans) so a volume alarm sees nothing; the orphan rate jumps immediately. This is the single check that catches the regression the day it ships instead of the next time someone tries to debug an incident and finds the trace useless.

### Your eval suite exists but hasn't blocked a deploy in months

*The symptom in one line: there is a green "evals passed" check, regressions still ship, and nobody can remember the last time the gate actually stopped anything.*

The chapter is blunt that the winning move is *"the smallest eval kit that actually runs on every change."* The failure is the slow death of that "actually runs." It decays three ways, all quiet. The suite gets *flaky* — an LLM-as-judge that scores 7/10 one run and 5/10 the next trips the threshold at random, so the team sets it to non-blocking "until we stabilize it" and never re-arms it. Or it gets *slow* — replaying 200 traces takes twenty minutes, so it moves to a nightly job that nobody reads instead of a pre-deploy gate that blocks. Or it goes *stale* — the corpus was captured six months ago, the product moved on, and the suite now passes everything because it tests behavior the agent no longer exhibits. In all three cases the green check is theater: it provides the *feeling* of a safety net with none of the function.

The fix is to treat the gate's own health as a first-class signal. Track the **eval block rate** — the fraction of deploys the gate actually stopped over a rolling window — and review it: a gate that has blocked *zero* deploys in months is either guarding nothing or has been quietly disabled, and both deserve a look. Keep the blocking suite small and fast enough that it stays on the deploy path (the chapter's 10–50 real-workload inputs, deterministic scoring where possible) and push the slow, large, flaky evals to the continuous out-of-band loop where flakiness costs nothing. Fight staleness with the eval-governance discipline from earlier in this chapter — version the corpus, refresh it from recent production traces on a schedule — and add a **judge-stability check**: before trusting an LLM-as-judge to gate, score the same fixed output five times and confirm the variance is small enough to threshold against. An eval gate you cannot show has blocked a real regression is a curator that never runs (Ch.07) wearing a different costume.

### A secret leaks into your traces through a path the redactor never saw

*The symptom in one line: an API key or a user's email shows up in plaintext in your trace backend, often a third-party SaaS you don't control.*

The chapter's redaction section covers the policy — secrets, PII, model text, redacted at the exporter before spans ship. The failure is never the path you instrumented; it is the path you forgot. A new tool returns a payload with an OAuth token nested three levels deep in a field your pattern-matcher doesn't walk. An *exception* gets recorded on a span via `recordException`, and the stack trace or error message contains the very credential that caused the failure. A model echoes a secret it was given back into its output, and your "we only record token counts" rule had an exception for one debug flag that shipped to prod. Once any of these reaches a third-party trace backend, the secret is *exfiltrated to a system you don't control* — rotating it is now mandatory, and you can't un-send it.

The fix is **defense in depth at the trace boundary, not a single regex**. Redact in an in-stream processor in the collector (the chapter's right answer) *and* default span attributes to an allowlist — only named, vetted fields are recorded verbatim; everything else is dropped or hashed, so a new tool's nested token is excluded by default rather than included by default. Treat `recordException` as a redaction surface in its own right: scrub messages and stack frames before they attach to a span. Then *prove it stays fixed* with a **canary secret**: inject a known fake credential into a tool argument on a synthetic run, and alarm if that exact string ever appears in the trace sink. That single test, run continuously, converts "we think redaction works" into "redaction demonstrably works as of the last run" — the same instinct as Ch.18's adversarial testing, pointed at your own telemetry.

### Your dashboard has thirty graphs and nobody trusts any of them

*The symptom in one line: an incident happens, the dashboards all look fine, and the on-call engineer debugs from raw logs instead — because the alarms cried wolf too many times.*

The chapter warns against this directly — *"a dashboard with twenty graphs that nobody trusts is worse than one graph that catches every incident"* — but it frames it as a build-order mistake. The production shape is *alarm fatigue*, and it sets in even when you built the layers in the right order. Every metric in the catalog gets a threshold alarm, each tuned in isolation; together they fire dozens of times a week on normal variance. The on-call engineer learns that alarms are noise, silences the channel, and the *one* alarm that mattered — the cost anomaly, the cache-hit collapse, the subagent-success cliff — scrolls past unread. The dashboard didn't fail technically; it failed socially, which is harder to fix and easier to ignore.

The fix is to **alarm on symptoms users feel, not on every metric you happen to collect**. Borrow the discipline of *page vs. ticket vs. dashboard-only*: a signal pages a human only if it means a user is being harmed right now (error rate spiking, latency past SLO, cost anomaly past the chapter's 3× rule); it opens a ticket if it needs attention this week (cache hit ratio drifting down, curator action count flatlining); it lives on the dashboard with no alarm at all if it's context you consult during an incident but never gives a clean threshold. Most of the metrics catalog is in that third tier — they are *diagnostic*, read after a page, not themselves page-worthy. Then measure the alarms the way you measure the agent: track each alarm's **precision** (what fraction of firings led to a real action) and delete or widen any alarm that sits below roughly half. An alarm nobody acts on is worse than no alarm, because it teaches the team to ignore the channel where the real one will eventually appear.

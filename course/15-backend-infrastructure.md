...
The chapter above is the architecture. This section is what still breaks once that architecture is carrying real traffic — the failures that wake an on-call engineer, and the pattern that resolves each. They are ordered by how often they bite, not by how interesting they are: the first two land on almost every backend the first week it sees load above a demo; the last two start to matter once you run across multiple machines.

### The queue backs up and everything slows to a crawl

*The symptom in one line: runs that used to start instantly now sit "queued" for minutes, and users start complaining before any dashboard turns red.*

This is the single most common backend incident, and it is almost never a code bug. A traffic spike, a slow model provider, a tool that suddenly takes thirty seconds instead of three — any of these makes work arrive faster than the worker pool drains it, and the durable queue does exactly what it is built to do: it absorbs the backlog without dropping anything. That is the trap. Because nothing errors and nothing is lost, the only visible symptom is *latency*, and queue wait time is the one number most teams forget to graph. The chapter's advice to size the worker pool against the model provider's rate limit is the right starting point, but a fixed pool sized for the median is by definition undersized for the spike.

The fix is to treat **queue depth and queue wait time as first-class SLO metrics**, not afterthoughts. Graph both, per queue and per tenant, and alarm on *oldest-message age* (the wait time of the front of the queue) rather than raw depth — depth is noisy, age tells you directly how long a user is staring at a spinner. A working threshold to start from: page when the oldest queued run exceeds your interactive-latency budget (often a handful of seconds for chat, much longer for batch). The operational lever the chapter gestures at but does not name is **backpressure**: when the queue's oldest-message age crosses a ceiling, stop accepting new interactive work at the API boundary and return a 429 with a `Retry-After` (the per-tenant token bucket from the rate-limit section is where this lives) instead of silently enqueueing into a backlog the user will wait out anyway. Rejecting fast is a better user experience than queueing forever, and it sheds the load that lets the pool catch up. Autoscaling the worker pool on queue age is the durable fix; backpressure is what holds the line until the new workers come up.

### A missing WHERE clause leaks one tenant's data into another's

*The symptom in one line: a user sees a run, a transcript, or a cost number that belongs to a different customer.*

In a row-scoped multi-tenant store (every table tagged with `tenant_id`, every query expected to filter on it), this is the highest-severity failure the backend can produce, and it is depressingly easy to ship. One developer writes `SELECT * FROM runs WHERE id = ?` without the tenant predicate, the code reviewer doesn't catch it, the tests pass because the test database has one tenant, and the leak only manifests in production once two tenants' rows coexist. The cause is structural: tenant scoping enforced by *developer discipline on every query* will eventually be defeated by the one query that forgot. The chapter lists the mitigations; the failure case is what happens when you treat them as optional.

The fix is to make tenant scoping **impossible to forget rather than easy to remember**. Default-deny at the storage layer: route every query through a data-access layer that requires a tenant context and *errors* — never silently returns all rows — when the context is absent, so an unscoped query fails closed in development instead of leaking in production. Where the database supports it, back this with row-level security (a `tenant_id` policy enforced by the engine itself, beneath the application) so even a hand-written query that forgets the `WHERE` returns nothing. Then prove the boundary continuously with a **synthetic cross-tenant canary**: a always-on probe that writes a known row as tenant A, queries it as tenant B, and *pages* if it ever sees a non-zero result — this is a monitoring alarm, not just a unit test, and it is the difference between catching a leak in the minute it ships versus the day a customer reports it. Remember the audit log is a tenant-scoped store too (Ch.05); a leak there is as severe as one in the run table, and operator views must default to scoped, not global. This is the storage half of the contract Ch.18 owns end-to-end.

### A client reconnects and the progress stream has a hole in it

*The symptom in one line: a user's phone sleeps for ten seconds, the connection drops, and when it reconnects the UI is missing the tool calls and approvals that happened in the gap.*

The chapter's decoupling-via-bus design is what makes the run survive a client disconnect — the worker keeps going, events keep landing on the bus. But survival of the *run* is not the same as continuity of the *stream*. The default naive setup pushes events to whatever connection is open at the moment the event fires; if the client is mid-reconnect, that event is gone for that client. Mobile networks drop connections constantly, load-balancer idle timeouts cut long-lived SSE connections on a schedule, and a laptop lid closing is a disconnect. So this is not an edge case — it is the steady state of any client that isn't on a wired connection, and it shows up as users who swear the agent "skipped a step" when it did the step and they just never saw the event.

The fix is a **resumable event stream backed by a per-run event log, not a fire-and-forget pub/sub**. Persist every emitted event with a monotonic sequence number (the `run_events` rows you are already writing at each step boundary double as this log), and have the client send the last sequence number it saw on reconnect — the SSE `Last-Event-ID` header is the standard mechanism; a WebSocket carries an equivalent cursor in its resume message. The server replays everything after that cursor from the log, then resumes live tailing. The anti-pattern to ban is **treating the live bus as the source of truth for client state**: the bus is for low-latency delivery, the durable event log is for correctness, and a client's view must always be reconstructable from the log alone. Watch **reconnect-gap rate** — the fraction of reconnects where the client's last-seen cursor lags the run's current sequence by more than a threshold — as the signal that your buffering or replay window is too short.

### Two schedulers fire the same cron job twice

*The symptom in one line: a scheduled review runs twice, a user gets two identical proactive messages, or a heartbeat dispatches every due run in duplicate.*

The chapter is explicit that the scheduler must be a singleton at scale and that leader election via a CAS lock row is the pattern. The failure shows up the moment that election is subtly wrong — and it usually is, in one of two ways. Either two processes both believe they hold the lock because the lock has no expiry and the previous leader died without releasing it (now you have a permanent split-brain and *every* tick double-dispatches), or the lock *has* a TTL but it is shorter than a scheduler tick takes to run, so the lease expires mid-tick, a second process claims it, and both finish the same batch. The result is the same: due work gets enqueued twice. If the downstream is idempotent the damage is contained; if it sends a message or charges a card, you have shipped duplicates to a user.

The fix has two halves. First, give the leader lock a **TTL comfortably longer than the worst-case tick duration** and have the leader *refresh* it on a timer well inside that TTL — the same heartbeat discipline the orphan reaper uses (Ch.08). A lock that the holder must keep renewing means a dead leader's lock expires on its own and a live leader never loses one mid-tick. Second — and this is the belt to the lock's suspenders — make **enqueue itself idempotent on a per-tick dedupe key**: derive a deterministic key from `(job_id, scheduled_wake_time)` and let the queue reject a second enqueue of the same key, so even a brief double-leader window cannot produce double dispatch. Leader election reduces the probability of a duplicate; the dedupe key makes the duplicate harmless when election fails anyway, which it eventually will. Watch the count of **enqueues rejected by the dedupe key** — a nonzero, growing number is your early warning that the election is flapping before any user sees a duplicate.

...
The chapter above is the design. This section is what still breaks once that design is running and real traffic is paying the bill — the failures you actually get paged for, and the pattern that resolves each. They are ordered by how often they bite, not by how interesting they are: the first two go wrong on nearly every agent the week it ships real volume; the last three start to matter once you are routing across profiles, escalating on quality, or trusting a downgrade to save money.

### Everything quietly runs on the expensive model

*The symptom in one line: the bill is several times your estimate, and a trace shows compaction, classification, and title generation all going through the same model as the main answer.*

The fastest way to get here is the most boring one: someone wires a single model ID at the top of a config, every call reads it, and the routing cascade from this chapter never gets built because nothing is visibly broken. Compaction (Ch.05), the *"is this a question or a command?"* classifier, the session-title call, the background memory review (Ch.07) — all of them run on the deep, reasoning-capable model that should be reserved for the hard final answer. The output is fine, which is exactly why nobody looks; the only signal is a bill two-to-five times what the workload warrants. The cause is that the auxiliary tier was never separated from the main agent, so cheap work pays main-agent prices on every turn.

The fix is to make the auxiliary tier a hard architectural boundary, then prove it with a metric. Route compaction, summarization, classification, and title/slug generation through a separate cheap profile — and instrument **cost-by-call-purpose**: tag every model call with its purpose (`main`, `compaction`, `classify`, `title`, `review`) and break the daily cost ledger (Ch.16) down by that tag. The anti-pattern to alarm on is *auxiliary cost as a fraction of total* climbing past a small ceiling — if compaction and classification together are more than a fraction of your spend, something cheap is running on the expensive model. This is the second-biggest cost lever after caching, and it is the one most often left on the table because the symptom is a number on an invoice, not an error in a log.

### Your prompt cache silently stops hitting

*The symptom in one line: cost per turn creeps up week over week with no feature change, and the cache-read share of input tokens has quietly fallen toward zero.*

Caching is the biggest cost lever, and it fails *silently* — there is no error when a cache miss happens, only a slightly larger bill. The usual causes are mundane: someone rebuilds the system prompt with a timestamp or a per-request field so the prefix is no longer byte-stable (Ch.04); a routing change starts switching the main model mid-session, so every switch pays a full cache miss; the request cadence for a tenant goes sparse and entries fall out of the provider's retention window before they are reused. Any one of these turns multi-session amortization off, and because the output is identical, nothing pages until the monthly cost review.

The fix is to treat the cache as a metric you watch, not a feature you assume. Track **cache-read share** — `cacheReadInputTokens / (freshInputTokens + cacheReadInputTokens)` from the normalized usage shape — per tenant, and alarm when it drops below the floor your prefix design should sustain (for a long stable system prompt, that floor is high; a sudden collapse means the prefix churned or the model started rotating). The named anti-pattern is the **mid-session model swap**: letting the main agent rotate per call is the single most common cause of cache collapse, and the cure is the per-run discipline from this chapter — pick the main model at session start and keep it fixed; route per-call only the auxiliary and router-shaped calls where cache compounding was never the point. When you do change the prefix on purpose, expect a one-time cache-write cost and watch the read share recover over the next few sessions; if it does not recover, the prefix is not actually byte-stable.

### Your token estimate disagrees with the provider's bill

*The symptom in one line: pre-call budgets pass, runs complete, and yet the tenant blows their budget — or you reject requests that would have fit.*

The pre-call token count is doing two jobs — rejecting over-budget requests and triggering compaction before overflow — and both depend on the count matching what the provider actually bills. It usually doesn't, for two reasons that compound. First, your local tokenizer estimate drifts from the provider's true count, especially on tool schemas, system-prompt scaffolding, and non-English text; an estimate that is 10% low lets over-budget requests through, and one that is 10% high rejects requests that would have fit. Second, and worse, provider usage reports disagree on what `input_tokens` includes — some fold cached tokens into the input total, some report them separately, some add line items for reasoning or tool tokens — so a cost computed from the raw response double-counts on one provider and under-counts on another, and every budget decision downstream inherits the error.

The fix is the **normalize-at-the-adapter-boundary** discipline, made measurable. Each provider adapter (Ch.11) emits the `NormalizedUsage` shape — fresh, cache-read, cache-write, output as separate fields — and the cost formula never touches a raw provider response. Then close the loop on the estimate itself: log estimated tokens alongside the provider's reported tokens for every call and track the **estimate-vs-actual error** as a metric (Ch.16); when its rolling distribution drifts past a few percent on either side, your tokenizer or your safety buffer needs retuning. Size the pre-call safety buffer from that measured error, not a guessed constant — OpenCode's `usable() = context_limit − max_output − safety_buffer` is only as good as the buffer, and a buffer set by feel is the difference between catching overflow cheaply before the call and paying for a `prompt_too_long` round-trip after it.

### Quality escalation keeps re-running the same wrong answer

*The symptom in one line: a step that fails its check retries several times, costs three times as much, and returns the same insufficient output it did the first time.*

There are two distinct failure classes — a transient infrastructure error (429, 5xx, network) and a successful call whose output failed a downstream check — and conflating them is a common, expensive bug. When a cheap model returns a structurally valid but inadequate answer and the system treats that as a transient failure, it retries *the same model with the same prompt*, which deterministically produces the same inadequate answer; you pay N times for one bad result and add latency the user feels. The reverse error is just as costly in latency: treating a real 429 as a quality failure and "escalating" to a stronger, more expensive profile when a plain backoff-and-retry on the original profile would have worked.

The fix is to keep the two mechanisms structurally separate, and to bound the expensive one. **Transient retry** uses backoff and ultimately the provider fallback chain (Ch.15's rate-limit cascade), targeting the *same* output. **Quality escalation** moves to the next stronger profile exactly once per step and targets a *better* output — it is not a retry loop. Cap escalation at a single hop (`balanced` → `deep`, then stop and surface the failure), and emit a `model.escalated` trace event (Ch.16) on every hop carrying `from`, `to`, and `reason`. Then watch the **escalation rate** per step type: a step that escalates on most calls is mis-routed at Level 1 — its baseline profile is too weak and should be promoted permanently rather than escalated every single time, which pays the cheap call *and* the expensive one on every request.

### Downgrading to save money silently wrecks quality

*The symptom in one line: a cost-anomaly response or a budget overrun quietly routes a tenant to a cheaper profile, and quality regresses in a way no alarm catches.*

Downgrade-on-overrun is a legitimate budget policy — but it is *one* policy, not a safe default, and applying it blindly is how a cost contract silently breaks a quality contract. Two shapes of this bite in production. The narrow one: a forecast exceeds remaining budget, the router downgrades a regulated-data or high-stakes legal-review workflow to a weaker model, and you have now silently violated an agreement that required the stronger one. The broad one: a step is *permanently* demoted from `balanced` to `fast` to save money with no eval behind the decision, and a subtle quality regression — worse extraction, more hallucinated structure, missed edge cases — ships to every user because the cheaper output still looks plausible.

The fix has two parts. First, make downgrade a **per-tenant budget policy the router reads**, never a hardcoded fallback: some tenants prefer to *block* and request budget approval (regulated data, high-stakes review), others prefer to *downgrade* and surface the trade-off in the UI (interactive chat, exploratory work). The router enforces the policy; it does not choose for the tenant. Second, gate every *permanent* profile change behind **eval-gated promotion** (this chapter's pattern, built on Ch.16's eval pillar): replay a representative trace corpus through the candidate profile, score outcomes with a deterministic comparison or an evaluator subagent (Ch.10's verification pattern), and promote only on no critical regression — *per tenant*, because a profile that holds for one workload regresses on another. A cheaper model is only cheaper if its output is still good enough; the eval gate is what tells you whether "good enough" survived the downgrade, and it is the difference between a real saving and a regression you discover from a churned customer.
